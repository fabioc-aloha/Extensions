...
Type a domain name, choose a format, and copy or insert the resulting reference. Use your own Brandfetch or Logo.dev API key; credentials remain in VS Code SecretStorage.
...
| **Provider fallback** | Logo.dev is used when Brandfetch returns no logo and a Logo.dev key is configured |
| **In-memory cache** | Repeated lookups within a session avoid another network call |
| **Secure key storage** | Provider API keys are stored in VS Code SecretStorage, never in settings |
| **Right-click menus** | Right-click anywhere in an editor → **Fetch Logo** or **Insert Logo at Cursor** |

## Requirements

A [Brandfetch API key](https://brandfetch.com/developers) or a [Logo.dev API key](https://logo.dev) is required. Configure either provider through **Brandfetch: Set API Key**.
...
| `Brandfetch: Set API Key` | Palette | Store a Brandfetch or Logo.dev API key in VS Code SecretStorage |
| `Brandfetch: Clear Logo Cache` | Palette | Clear the in-memory cache |
